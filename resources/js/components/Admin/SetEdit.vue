      ...
      <div class="flex flex-col px-5">
        ...
        <form @submit.prevent="">
          ...
          <div class="px-3 mb-2">
            ...
            <span
              class="p-2 inline-block text-sm capitalize"
          ...
          <div v-for="(item, i) in effects" :key="i">
...
export default {
  ...
  data() {
    return {
      ...
      NO_OF_PARTS_NEED: new RegExp("[0-9$]+"),
      INDEX_NO_OF_PARTS: 0,
    };
  },
  methods: {
    ...
    removeField(index) {
      this.effects = this.effects.filter((data, i) => i !== index);
    },
    submitSet() {
      let selectedItems = [];
      ...
      this.formError = [];
      axios
        .put(`/admin/set/${this.set.id}/edit`, data)
        .then((response) => {
    ...
    getAttribute(data) {
      return data
        .replace(this.NO_OF_PARTS_NEED, "")
        .replace(/_/gi, " ")
        .replace(/^DST/, "")
        .replace(/\d+/gi, "")
        .trim();
    },
    initializeSetEffects(effects) {
      effects = JSON.parse(effects);
      for (const [attribute, value] of Object.entries(effects)) {
        let no_of_parts = `${
          attribute.match(this.NO_OF_PARTS_NEED)[this.INDEX_NO_OF_PARTS]
        } / ${this.selectedItems.length}`.trim();
  ...
  created() {
    ...
    this.selectedItems = this.set.items;
    this.initializeSetEffects(this.set.effects);
  },
  watch: {
    search(to) {
      if (!to) {
