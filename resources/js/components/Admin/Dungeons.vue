      ...
      <div class="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
        <div>
          <p class="text-sm text-gray-700 tracking-wide">
            Showing
            <span class="font-medium"> {{ dungeons.from }}</span>
            to
            <span class="font-medium"> {{ dungeons.to }}</span>
            of
            <span class="font-medium">{{ dungeons.total }}</span>
            results
          </p>
        </div>
        <div>
          ...
          >
            <a
              :class="
                dungeons.prev_page_url == null
                  ? 'cursor-not-allowed bg-gray-50'
                  : 'cursor-pointer bg-white'
              "
              v-on:click="dungeons.prev_page_url ? prev() : '#'"
            ...
            >
              <option
                :key="index"
                v-for="(page, index) in dungeons.last_page"
                :value="page"
              >
                page : {{ page }}
              </option>
            ...
            <a
              :class="
                dungeons.next_page_url == null
                  ? 'cursor-not-allowed bg-gray-50'
                  : 'cursor-pointer bg-white'
              "
              v-on:click="dungeons.next_page_url ? next() : '#'"
...
export default {
  ...
  methods: {
    get() {
      axios.get("/admin/records/dungeon").then((response) => {
        this.dungeons = response.data;
      });
    },
    searchMonster() {
      this.dungeonsBeforeSearch = this.dungeons;
      axios.get(`/admin/dungeon/search/${this.search}`).then((response) => {
        this.dungeons = response.data;
      });
    },
    changePage() {
      if (this.dungeons.current_page !== this.selectedPage) {
        axios
          .get(`${this.dungeons.path}?page=${this.selectedPage}`)
          .then((response) => (this.dungeons = response.data));
      }
    },
    prev() {
      axios.get(this.dungeons.prev_page_url).then((response) => {
        this.selectedPage--;
        this.dungeons = response.data;
      });
    },
    next() {
      axios.get(this.dungeons.next_page_url).then((response) => {
        this.selectedPage++;
        this.dungeons = response.data;
      });
    },
  ...
  watch: {
    search(to) {
      if (!to) {
        this.dungeons = this.dungeonsBeforeSearch;
      }
    },
  },
};
